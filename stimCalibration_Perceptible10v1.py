#Stimulus Calibration Script for Perceptible shock
#Jo Stasiak 2022

'''
Script for Perceptible Shock Calibration
...
expName = 'PerCalibration V1'
userVar = {'Participant':'Enter subject ID'}#Make sure this is the same ID used through all calibrations
...
#function for showing the instructions on screen
def instFunc(text):
    win1.setMouseVisible(False)
...
detectVal=2
rating=2
detectq = visual.TextStim(win1, text='', height=0.09, color='white', pos=[0,0.3])
yq=visual.TextStim(win1, text=y, height=0.08, color='black', pos=[-0.25, -0.15])
nq = visual.TextStim(win1, text=n, height=0.08, color='black', pos=[0.25, -0.15])
def detect():#Ask participants if they can reliably detect the shock
    ...
    while continueRoutine:
        ...
        if button_resp.status == STARTED:
            ...
            if button_resp.newButtonState != button_resp.oldButtonState:
                ...
                button_resp.newPressedButtons = [i for i in [0,1] if i in button_resp.pressedButtons]#the first 2 buttons on the joystick are used to respond 'Yes' or 'No'
    ...
    return detectVal#return their response
...
ratingArray=[0]
ratingVal = 2
ratingInstr = visual.TextStim(win1, text= 'Was this shock at all unpleasant?', height=0.08, color='white', pos=[0,0.3])
...
win1.setMouseVisible(False)
instFunc(text='''You will now go through testing to identify a perceptible level of electrical stimulation that will be used during the task.''')
instFunc(text='''An electric shock will be delivered to your wrist. You will answer if you can reliably detect the shock and whether it was at all unpleasant.''')
instFunc(text='''We will start with low levels of stimulation, and the intensity will increase or decrease slowly based on your ratings.''')
instFunc(text='''Do you have any questions?''')


fileHandle = open('data/'+subID+'_PerceptibleCalibration_1.txt','w')#Create a new text file for this calibration
curIntensity=2400#starting intensity of 2 volts
win1.setMouseVisible(False)
curRating = 0
intensityDelta = 0
perceptibleShock = 0
...
allData = pd.read_table('data/'+subID+'_PerceptibleCalibration_1.txt', sep='\t', header=None)# save the text file
